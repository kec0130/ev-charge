...
test('provider client validates and paginates API results', async (t) => {
  ...
  let respond: AxiosAdapter = async () => {
    ...
    respond = async (config) => {
      ...
      return {
        ...
        data: {
          resultCode: '00',
          totalCount: 2,
          items: { item: { statId: String(pageNo) } },
        },
      };
    };
  ...
  await t.test('queries current provider codes for legacy administrative regions', async () => {
    const codes: string[] = [];
    respond = async (config) => {
      codes.push(config.params.zscode);
      return {
        config,
        status: 200,
        statusText: 'OK',
    ...
    for (const code of ['29110', '46230', '46770', '45110', '12210', '26170']) {
      await getChargersAPI(code);
    }
    assert.deepEqual(codes, ['12210', '12190', '12740', '52110', '12210', '26170']);
  });
