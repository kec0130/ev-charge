...
import { buildStations, filterStations, getStationAccess, isValidCoord } from '../src/utils/stations';
...
function charger(overrides: Partial<ChargerDTO> = {}): ChargerDTO {
  return {
    statId: 'public', statNm: '공영주차장', chgerId: '01', chgerType: '04',
    lat: '37.56', lng: '126.97', addr: '서울', location: '', useTime: '24시간',
    busiId: '', bnm: '', busiNm: '', busiCall: '', stat: '2', statUpdDt: '',
    lastTsdt: '', lastTedt: '', nowTsdt: '', powerType: '', output: '50',
    method: '단독', zcode: '11', zscode: '11140', kind: '', kindDetail: '',
    parkingFree: 'N', note: '', limitYn: 'N', limitDetail: '', delYn: 'N',
    delDetail: '', trafficYn: 'N', ...overrides,
  };
}
...
test('default map excludes restricted, missing and conflicting access information', () => {
  ...
  assert.deepEqual(filterStations(data, defaults).stations.map(s => s.statId), ['public']);
...
test('deleted chargers, duplicate records and invalid map positions are omitted', () => {
  const data = buildStations([
    charger(), charger(), charger({ chgerId: '02', delYn: 'Y' }),
...
test('available and fast must match the same charger, with an accurate marker', () => {
  const mixed = buildStations([
    charger({ stat: '3' }),
    charger({ chgerId: '02', chgerType: '02', stat: '2' }),
  ], origin);
  ...
  assert.equal(filterStations(mixed, {
    ...defaults, onlyAvailable: true, onlyFastCharger: true,
  }).stationCount, 0);
...
test('available fast chargers appear when both filters are enabled', () => {
  ...
  const filtered = filterStations(data, { ...defaults, onlyAvailable: true, onlyFastCharger: true });
...
test('empty results are valid and station distances stay sorted', () => {
  ...
  assert.deepEqual(data.stations.map(s => s.statId), ['public', 'far']);
});
